...
class SimpleSAML_SessionHandlerPHP extends SimpleSAML_SessionHandler {
	...
	public function newSessionId() {
		...
		// Don't regenerate session_id if set.  This would lose
		// Joomla state and cause saving from SSP to fail
		// since Joomla expects to control insertion of
		// sessions into its database.
		if (session_id() !== '')
			return;
	...
	public function saveSession(SimpleSAML_Session $session) {
		// we need to restart and save the session,
		// because Joomla's shutdown handler will have
		// written and closed it already.  We'll reuse
		// the existing session_name and session_id.
		session_start();
